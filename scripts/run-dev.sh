...
if [ ! -d "./backend" ]; then
    echo -e "${RED}[ERRO] Pasta backend nao encontrada!${NC}"
    exit 1
fi

if [ ! -d "./frontend" ]; then
    echo -e "${RED}[ERRO] Pasta frontend nao encontrada!${NC}"
    exit 1
fi

# Funcao para iniciar backend
start_backend() {
    cd backend
...
echo ""
echo -e "${GREEN}[OK] Backend iniciado na porta 8000${NC}"
echo -e "${GREEN}[OK] Frontend iniciado na porta 3000${NC}"
echo ""
