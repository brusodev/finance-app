...
if [ ! -d "$BACKEND_PATH" ]; then
    show_error "Pasta backend nao encontrada!"
    exit 1
fi
...
if ! command -v python3 &> /dev/null; then
    show_error "Python3 nao esta instalado ou nao esta no PATH"
...
if [ ! -d "venv" ]; then
    ...
    show_success "Ambiente virtual ja existe"
fi
...
if [ $? -eq 0 ]; then
    ...
    show_error "Erro ao instalar dependencias Python"
    exit 1
fi
...
EOF
    show_success "Arquivo .env criado (configure as variaveis!)"
    show_info "Edite .env com suas credenciais PostgreSQL"
else
    show_success "Arquivo .env ja existe"
fi
...
if [ ! -d "$FRONTEND_PATH" ]; then
    show_error "Pasta frontend nao encontrada!"
    exit 1
fi
...
if ! command -v node &> /dev/null; then
    show_error "Node.js nao esta instalado ou nao esta no PATH"
...
if ! command -v npm &> /dev/null; then
    show_error "npm nao esta instalado"
    exit 1
fi
...
if [ ! -d "node_modules" ]; then
    show_info "Instalando dependencias Node.js (pode levar alguns minutos)..."
    npm install
    
    if [ $? -eq 0 ]; then
        show_success "Dependencias Node.js instaladas"
    else
        show_error "Erro ao instalar dependencias Node.js"
        exit 1
    fi
else
    show_success "Dependencias Node.js ja existem"
fi
...
show_section "Informacoes sobre o Banco de Dados"

show_info "Este projeto usa PostgreSQL"
show_info "Proximas etapas:"
...
echo "  4. Execute migracoes (quando implementadas)"

# ==============================================================================
# RESUMO FINAL
# ==============================================================================

show_section "Setup Concluido! Pronto para usar"
...
echo -e "${CYAN}==========================================${NC}"
echo -e "${GREEN}Pronto para comcar a desenvolver! Sucesso!${NC}"
echo -e "${CYAN}==========================================${NC}"
echo ""
