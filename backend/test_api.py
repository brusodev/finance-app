...
def test_users(user_id):
    ...
    print_result("GET /users/99999 - Não encontrado (deve falhar)", response, 404)
...
def test_categories():
    ...
    response = requests.post(f"{BASE_URL}/categories/", json={"name": "Alimentação"})
    ...
    response = requests.put(f"{BASE_URL}/categories/{category_id}", json={"name": "Comida"})
    ...
    response = requests.post(f"{BASE_URL}/categories/", json={"name": "Transporte"})
